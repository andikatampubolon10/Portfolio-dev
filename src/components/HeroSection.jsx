...
const OrbitingDecoration = ({ icon: Icon, delay, className, isRevealed, enableAmbientMotion }) => (
  <Gsap.div
    ...
    className={`absolute flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-full border border-sky-500/20 bg-white/60 backdrop-blur-lg shadow-[0_10px_30px_rgba(132,204,22,0.12)] ${className}`}
    style={enableAmbientMotion && isRevealed ? {
      animation: `hero-float 5.8s ${delay + 0.35}s ease-in-out infinite`,
      willChange: 'transform',
    } : undefined}
  >
    <div className="absolute inset-0 rounded-full bg-gradient-to-br from-sky-300/25 to-transparent" />
...
const HeroSection = memo(function HeroSection({ isRevealed = true }) {
  ...
  return (
    <header
      ref={containerRef}
      id="hero-section"
      className="min-h-[100svh] w-full relative bg-[#FAF9F6] selection:bg-sky-300 selection:text-black overflow-hidden flex flex-col items-center justify-center pt-16 pb-16"
    >
      ...
      >
        ...
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_0%,rgba(56,189,248,0.12),transparent_48%),linear-gradient(to_bottom,rgba(56,189,248,0.04),transparent_48%)]" />
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[90vw] h-[90vw] max-w-[920px] max-h-[920px] rounded-full border border-sky-500/10" />
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[72vw] h-[72vw] max-w-[720px] max-h-[720px] rounded-full border border-sky-500/10" />

        {/* 3. Dynamic Organic Glowing Orbs — CSS animations for zero JS overhead */}
        <div
          className="absolute top-1/2 left-1/2 w-[60vw] h-[60vw] max-w-[800px] max-h-[800px] bg-sky-300 rounded-full blur-[90px] lg:blur-[130px] opacity-[0.1]"
          style={enableAmbientMotion && isRevealed ? {
        ...
        />
        <div
          className="absolute top-1/4 right-[20%] w-[40vw] h-[40vw] max-w-[600px] max-h-[600px] bg-sky-400 rounded-full blur-[90px] lg:blur-[120px] opacity-[0.06]"
          style={enableAmbientMotion && isRevealed ? {
        ...
        />
        <div
          className="absolute bottom-[10%] left-[20%] w-[45vw] h-[45vw] max-w-[650px] max-h-[650px] bg-sky-200 rounded-full blur-[100px] lg:blur-[130px] opacity-[0.08]"
          style={enableAmbientMotion && isRevealed ? {
      ...
      >
        ...
        >
          <h2 className="text-[clamp(1.35rem,4.2vw,2.25rem)] font-bold text-black/80 tracking-tight flex items-center justify-center flex-wrap gap-2 px-2">
            Architecting <span className="bg-sky-400/30 px-2 rounded-md ring-1 ring-sky-500/20">Intelligent</span> Paradigms<span className="text-sky-500 font-extrabold -ml-1">.</span>
          </h2>
        ...
        >
          <button
            ...
            className="group flex items-center gap-2 bg-black text-white px-6 py-3 font-mono text-sm font-bold uppercase tracking-wider hover:bg-sky-400 hover:text-black transition-all duration-300 cursor-pointer"
          >
          ...
          <a
            href="/Andika Parlinggoman Tampubolon-resume (English).pdf"
            download
            className="group flex items-center gap-2 bg-transparent text-black border-2 border-black px-6 py-3 font-mono text-sm font-bold uppercase tracking-wider hover:bg-black hover:text-sky-400 transition-all duration-300"
          >
